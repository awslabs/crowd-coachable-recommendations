...
def convert_dev_data_to_msmarco(data, data_folder, model_name):
    writing_dir = os.path.join(
        data_folder, "eval", "dev_data_{}.tsv".format(model_name)
    )
...
def convert_ranking_to_msmarco(data, data_folder, model_name):
    writing_dir = os.path.join(
        data_folder, "eval", "rankings_{}.tsv".format(model_name)
    )
    with open(writing_dir, "w") as w:
        for qid in data:
...
if __name__ == "__main__":
    ...
    path_to_candidate = convert_ranking_to_msmarco(
        ranking_profile, data_folder, model_name_
    )
