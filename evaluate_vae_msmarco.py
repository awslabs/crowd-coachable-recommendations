...
def VAE_rerank_entropy(model, dev, queries, corpus,  
                    num_ent_sample=20,
                    embed_size=768,
                    batch_size=64,
    ...
    for i in range(num_item):
        ...
        with torch.no_grad():
            tokenized_source = tokenizer(source_sentence, truncation=True, padding='max_length', max_length=350, return_tensors = "pt")
            source_mean, source_std = model.forward(input_ids=tokenized_source["input_ids"].cuda(),
                                                    attention_mask=tokenized_source["attention_mask"].cuda(),
                                                return_mean_std=True)
            for step in range(num_candicates_batches):
                batch = candidates_sentences[step*batch_size:(step+1)*batch_size]
                tokenized_candidates = tokenizer(batch, truncation=True, padding='max_length', max_length=350, return_tensors = "pt")
                cand_mean, cand_std = model.forward(input_ids=tokenized_candidates["input_ids"].cuda(),
                                                    attention_mask=tokenized_candidates["attention_mask"].cuda(),
                                                return_mean_std=True)
                item_embed_mean[step*batch_size:step*batch_size+cand_mean.shape[0],:] = cand_mean
                item_embed_std[step*batch_size:step*batch_size+cand_std.shape[0],:] = cand_std
...
def VAE_rerank_unique_items(model, data_dev, queries, corpus,  
                    num_ent_sample=20,
                    embedding_dim=768,
                    batch_size=64,
                    model_checkpoint="distilbert-base-uncased"):
    ...
    with torch.no_grad():
        for step_q in range(num_query_batches):
            ...
            tokens = tokenizer(queries_batch, padding='max_length',max_length=350, truncation=True, return_tensors='pt')
            embedding_mean, embedding_std = model(input_ids=tokens['input_ids'].cuda(),
                                            attention_mask=tokens['attention_mask'].cuda(),
                                            return_mean_std=True)
            queries_embeddings_mean[step_q*batch_size:(step_q*batch_size+len(indices)), :] = embedding_mean.cpu()
            queries_embeddings_std[step_q*batch_size:(step_q*batch_size+len(indices)), :] = embedding_std.cpu()
            
        for step_p in range(num_passage_batches):
            if (step_p % 100) == 0:
                print('processing batch: {} | {}'.format(step_p, num_passage_batches))
            passage_batch = effective_corpus_list[step_p*batch_size:(step_p+1)*batch_size]
            tokens = tokenizer(passage_batch, padding='max_length',max_length=350, truncation=True, return_tensors='pt')
            embedding_mean, embedding_std = model(input_ids=tokens['input_ids'].cuda(),
                                            attention_mask=tokens['attention_mask'].cuda(),
                                            return_mean_std=True)
            passage_embeddings_mean[step_p*batch_size:(step_p*batch_size+len(passage_batch)), :] = embedding_mean.cpu()
            passage_embeddings_std[step_p*batch_size:(step_p*batch_size+len(passage_batch)), :] = embedding_std.cpu()
...
def VAE_full_retrieval_unique_items(model, item_df,
    ...
    for batch_i, batch in enumerate(eval_dataloader):
        ...
        tokenized_batch = tokenizer(batch,truncation=True,padding='max_length',max_length=350,return_tensors = "pt")
        with torch.no_grad():
            batch_mean, batch_std = model.forward(input_ids = tokenized_batch["input_ids"].cuda(),
                                attention_mask = tokenized_batch["attention_mask"].cuda(), return_mean_std=True)
        item_embed_mean[batch_i*batch_size:batch_i*batch_size+batch_mean.shape[0],:] = batch_mean
        item_embed_std[batch_i*batch_size:batch_i*batch_size+batch_std.shape[0],:] = batch_std
    ...
    for _ in range(num_of_recommendation):
        ...
        for i  in range(num_of_batch):
            ...
            batch_mean = item_embed_mean[i*batch_size:i*batch_size+batch_length,:]
            batch_std = item_embed_std[i*batch_size:i*batch_size+batch_length,:]
...
def plot_losses(beta, checkpoint=98360):
    data_dir = 'msmarco_VAE_model_{}/checkpoint-{}/trainer_state.json'.format(beta, checkpoint)
    losses = []
...
def measure_vae_embedding_loss(data, corpus, queries, model, batch_size=64):
    ...
    loss_quereis_total = 0.
    loss_passages_total = 0.
    with torch.no_grad():
        for step in range(num_batches):
            query_batch = query_sentences[step*batch_size:(step+1)*batch_size]
            passage_batch = passage_sentences[step*batch_size:(step+1)*batch_size]
            tokens_query = tokenizer(query_batch, padding='max_length',max_length=350, truncation=True, return_tensors='pt')
            tokens_passage = tokenizer(passage_batch, padding='max_length',max_length=350, truncation=True, return_tensors='pt')
            outputs_query = model(input_ids=tokens_query['input_ids'].cuda(), attention_mask=tokens_query['attention_mask'].cuda())
            outputs_passage = model(input_ids=tokens_passage['input_ids'].cuda(), attention_mask=tokens_passage['attention_mask'].cuda())
    ...
    print('Mean embedding loss query: {:.4f}, passage: {:.4f}'.format(loss_quereis_total, loss_passages_total))
