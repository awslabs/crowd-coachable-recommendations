...
def load_query(data_root):
    dataframe_train = pd.read_csv(os.path.join(data_root, 'queries.train.tsv'), sep='\t', header=None, names=['qid', 'TITLE'])
    dataframe_dev = pd.read_csv(os.path.join(data_root, 'queries.dev.tsv'), sep='\t', header=None, names=['qid', 'TITLE'])
...
def load_dev_top1000_data(data_root):
    df = pd.read_csv(os.path.join(data_root, 'top1000.dev'), sep='\t', header=None, names=['qid', 'pid', 'query', 'passage'])
...
def load_training_data(data_root='data/ms_marco'):
    ce_scores = pd.read_pickle(os.path.join(data_root, 'cross-encoder-ms-marco-MiniLM-L-6-v2-scores.pkl'))
...
def get_event_df(data_root='data/ms_marco'):
    ...
    for qid, pids in dataset.items():
        ...
        labels = [1.] * len([pids['pos_pid'][0]]) + [0.] * len(pids['neg_pid'])
        cand_items.append(candicates)
        multi_label.append(labels)
    event_df = pd.DataFrame({'USER_ID': USER_ID,
...
oracle_model = ccrec.models.bbpr.BertBPR(item_df, dataset, batch_size=batch_size,
