...
class VAEData(LightningDataModule):
    def __init__(self, item_df, tokenizer, batch_size=64, do_validation=True, masked=False,
                 truncation=True, padding='max_length', max_length=300, **kw):
        super().__init__()
...
class _DataModule():
    ...
    def setup_for_activate_learning(self, rime_dataset):
        ...
        self.i_to_ptr = [self.item_to_ptr[hist[0]] for hist in self._D.user_in_test['_hist_items']]
...
class VAEModel:
    def __init__(self, item_df, batch_size=10, model_name='distilbert-base-uncased',
                max_length=300, max_epochs=10, beta=2e-3,
                model_cls_name='VAEPretrainedModel', **kw):
        self.item_df = item_df
        ...
        self.tower = LitVAEModel(self.beta, tokenizer=self.tokenizer, model_cls_name=self.model_cls_name)
    ...
    def fit(self, train_data=None):
        ...
        train_dm = VAEData(train_data, self.tokenizer, self.batch_size, masked=None, max_length=self.max_length)
        trainer = Trainer(max_epochs=self.max_epochs, gpus=torch.cuda.device_count(),
        ...
        if not os.path.exists(self.tower._checkpoint.dirpath):  # add manual checkpoint
            ...
            torch.save(self.tower.state_dict(), self.tower._checkpoint.dirpath + '/state-dict.pth')
