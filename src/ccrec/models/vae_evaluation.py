...
def VAE_precision(
    ...
    for batch_i, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            batch_token_logits = model.forward(
                input_ids=tokenized_batch["input_ids"].cuda(),
                attention_mask=tokenized_batch["attention_mask"].cuda(),
            )
        item_embed[
            batch_i * batch_size : batch_i * batch_size + batch_token_logits.shape[0], :
        ] = batch_token_logits.clone()
...
def VAE_rerank_entropy(
    ...
    for i in range(num_item):
        ...
        candidates = event_df["ITEM_ID"][
            num_item + 4 * i : num_item + 4 * (i + 1)
        ].tolist()
...
def VAE_full_retrieval_unique_items(
    ...
    for _ in range(num_of_recommendation):
        ...
        for i in range(num_of_batch):
            ...
            batch_mean = (
                item_embed_mean[i * batch_size : i * batch_size + batch_length, :]
                .clone()
                .cuda()
            )
            batch_std = (
                item_embed_std[i * batch_size : i * batch_size + batch_length, :]
                .clone()
                .cuda()
            )
