...
from .agent import (DualAgent, EpsAgent, GreedyAgent, Agent, BoltzmannAgent,
                    LatentNoiseAgent, VAEAgent)
...
def _sanity_check(self):
    ...
    if self.event_df is not None:
        assert set(self.event_df.columns) >= {"USER_ID", "ITEM_ID", "TIMESTAMP", "VALUE"}, \
            "please verify the columns in event_df"
        if len(self.event_df) == 0:
            warnings.warn("we cannot handle cold-start users, please put something as user history events")
...
class InteractiveExperiment:
    ...
    def __post_init__(self):
        ...
        working_names = f'{self.working_models[0].__class__.__name__}-x{len(self.working_models)}'

        self.training_env = auto_env_select(
            self.train_user_df if self.train_user_df is not None else self.user_df,
            self.item_df, self.event_df, **{
        ...
        self.training_agents = [self._create_training_agent(m, e) for m, e in zip(
            self.working_models,
            self.epsilon if np.size(self.epsilon) > 1 else [self.epsilon] * len(self.working_models),
        )]
        if isinstance(self.baseline_models, list):
            self.testing_agents = [GreedyAgent(m, training=False) for m in self.working_models] + [
                                   GreedyAgent(m, training=False) for m in self.baseline_models]
        else:  # int
            self.testing_agents = [GreedyAgent(m, training=False) for m in self.working_models[:self.baseline_models]]

        self._logger = TensorBoardLogger('logs', f"{working_names}-{len(self.user_df)}-{len(self.item_df)}")
        self._logger.log_hyperparams({
            ...
            **{repr(m): m._logger.log_dir for m in self.working_models if hasattr(m, "_logger")}
        })
    ...
    def _create_training_agent(self, model, epsilon):
        if epsilon == 'dual':
            ...
            return DualAgent(model, training=True, alpha_ub=alpha_ub,
                             beta_lb=alpha_ub / 2, beta_ub=alpha_ub * 2)
            ...
            return LatentNoiseAgent(model, std=std, training=False)  # deprecate training mode
        elif epsilon == 'vae':
            return VAEAgent(model, item_df=self.item_df, training=False)  # deprecate training mode
        else:
