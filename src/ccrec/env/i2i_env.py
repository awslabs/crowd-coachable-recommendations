...
def get_role_arn():
    ...
    return [x['Arn'] for x in iam.list_roles(PathPrefix='/service-role/')['Roles']
            if 'AmazonSageMaker-ExecutionRole' in x['RoleName']][-1]
...
class I2IConfig:
    ...
    PublicWorkforceTaskPrice: dict = dataclasses.field(
        default_factory=lambda:  {'AmountInUsd': {'Dollars': 0, 'Cents': 1, 'TenthFractionsOfACent': 2}})
    autorun: bool = True
...
class ExpInfo:
    ...
    def __post_init__(self):
        ...
        self.s3_label_template = f"s3://{self.bucket}/{self.exp_name}/support/label-template.json"
        self.s3_ui_template = f"s3://{self.bucket}/{self.exp_name}/support/template.liquid"
...
class I2IEnv(Env):
    ...
    @property
    def prompt(self):
        return "Please pick ALL items that are similar to the given item." if self.multi_label else \
    ...
    def __post_init__(self):
        ...
        assert (self.test_requests['_hist_len'] > 0).all(), "require item histories for i2i labeling"
    ...
    def _upload_request(self, request, exp_info):
        ...
        s3.put_object(
            Bucket=self.bucket, Key=get_s3_key(exp_info['s3_input']),
            Body=request.reset_index().to_json(orient='records', lines=True))
        s3.put_object(
            Bucket=self.bucket, Key=get_s3_key(exp_info['s3_label_template']),
            Body=json.dumps({
        ...
        s3.put_object(
            Bucket=self.bucket, Key=get_s3_key(exp_info['s3_ui_template']),
    ...
    def _wait(self, exp_info):
        while True:
            try:
                s3.get_object(Bucket=self.bucket, Key=get_s3_key(exp_info['s3_output_manifest']))
                return
            except Exception:
                print('.', end='')
                time.sleep(5)
...
def download_labels(exp_info, request=None, verbose=True):
    try:
        obj = s3.get_object(Bucket=exp_info['bucket'], Key=get_s3_key(exp_info['s3_output_manifest']))
    except Exception:
        print(exp_info)
        raise
    ...
    for i, lab in enumerate(label):
        ...
        elif lab < num_valid_classes + 1:  # exclude nan labels due to GroundTruth failures
            multi_label[i, int(lab)] = 1  # include "none of the above" labels as hard negatives
...
class I2IImageEnv(I2IEnv):
    ...
    def image_format(self, x):
        ...
        cand_texts = [self.item_df.loc[candidate]['TITLE'] for candidate in x['cand_items']]
        ...
        if isinstance(given_text, shap._explanation.Explanation):
            plot_shap_values(0, 0.9, given_text, width=50, nrows=4,
        ...
        ax.text(0, 0.4, getattr(self, "prompt", "Please pick most similar"), fontsize=18, ha='left', va='center')
        ...
        for i, (image, text) in enumerate(zip(cand_images, cand_texts)):
            ...
            if isinstance(text, shap._explanation.Explanation):
                plot_shap_values(0, 0.9, text, width=16, nrows=6,
                                 fontsize=int(14 * min(4 / ncols, 1)))
            else:
                ax.text(0, 0.5, '\n'.join(wrap(f'{i+1}. ' + text, width=20)[:6]),
                        fontsize=int(14 * min(4 / ncols, 1)), va='center')
    ...
    def _upload_image(self, i_img_data, exp_info):
        ...
        img_data.seek(0)
        s3.put_object(
            Bucket=self.bucket, Key=get_s3_key(exp_info['s3_output']) + f'/images/{i}.jpg',
            Body=img_data, ContentType='image/jpeg')

    def _upload_request(self, request, exp_info):
        num_valid_classes = len(request.iloc[0]['cand_titles'])
        with multiprocessing.Pool() as pool:
            images = list(pool.map(self.image_format, tqdm.tqdm(request.to_dict(orient='records'))))
        Image.open(images[0]).show()

        with multiprocessing.Pool() as pool:
            pool.map(functools.partial(self._upload_image, exp_info=exp_info), enumerate(images))
        request = request.assign(**{
            "source-ref": [exp_info['s3_output'] + f'/images/{i}.jpg' for i in range(len(images))]
        })
...
def image_ui_template(multi_label, prompt):
    tag = 'crowd-image-classifier-multi-select' if multi_label else 'crowd-image-classifier'
    return f"""
<script src="https://assets.crowd.aws/crowd-html-elements.js"></script>
<crowd-form>
  <{tag}
