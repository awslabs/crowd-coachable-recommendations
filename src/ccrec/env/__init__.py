...
class HoldoutEnv(Env):
    ...
    def _invoke(self, request, D, step_idx):
        ...
        expl_candidate_index = request["cand_items"].apply(
            lambda x: [self._tokenize[y] for y in x]
        )
        ...
        multi_label = np.asarray(
            [
                [i2d.get(e, 0.0) for e in j]
...
class SimuEnv(Env):
    ...
    def _label_candidate_probs(self, cand_item_ids, oracle_score_ast):
        expl_candidate_index = cand_item_ids.apply(
            lambda x: [self._tokenize[y] for y in x]
        )
        oracle_score_tensor = oracle_score_ast.as_tensor(auto_device())

        J = torch.as_tensor(
            np.vstack(expl_candidate_index), device=oracle_score_tensor.device
        )
    ...
    def _label_post_processing(self, probs):
        ...
        if probs.sum(1).min() == 0:
            warnings.warn(
                "none of the exploration candidates are available with >=0 probs; defaulting to random labeling"
            )
            probs = probs.clip(1e-10, None)

        if self.soft_label == "greedy":
    ...
    def _invoke(self, request, D, step_idx):
        ...
        for i in range(0, len(oracle_score_ast), oracle_score_ast.batch_size):
            ...
            probs = self._label_candidate_probs(
                request["cand_items"].iloc[iloc], oracle_score_ast[iloc]
            )
