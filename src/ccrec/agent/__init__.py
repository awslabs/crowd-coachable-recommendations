...
class EpsAgent(Agent):
    ...
    def __call__(self, D, k):
        ...
        topk_indices = _assign_topk(S, k, device=auto_device()).indices.reshape(
            (len(D), k)
        )
        rand_indices = _assign_topk(R, k, device=auto_device()).indices.reshape(
            (len(D), k)
        )

        num_per_list = [
            int(np.ceil(k * (1 - self.epsilon))),
            int(np.ceil(k * self.epsilon)),
        ]
        out = np.vstack(
            [
                merge_unique([t, r], num_per_list, k)[0]
