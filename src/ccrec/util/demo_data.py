...
class DemoData:
    ...
    def __post_init__(self):
        if self.user_df is not None and not isinstance(self.user_df, pd.DataFrame):
            self.user_df = _pandas_read(f"{self.data_root}/{self.user_df}").set_index(
                "USER_ID"
            )

        if not isinstance(self.item_df, pd.DataFrame):
            self.item_df = _pandas_read(f"{self.data_root}/{self.item_df}").set_index(
                "ITEM_ID"
            )
        ...
        if self.expl_response is not None and not isinstance(
            self.expl_response, pd.DataFrame
        ):
    ...
    def run_rime(self, topk=5, device=auto_device(), model_name=None, **model_kw):
        ...
        if model_name is None:
            this = rime.Experiment(
                D, default_k_items_per_user=topk, default_c_users_per_item=0
            )
            this.run()
            ...
            recs = rime.metrics._assign_topk(
                score, topk, device=device
            )  # recommendations as a csr matrix
            metrics = rime.metrics.evaluate_assigned(
                D.target_csr, recs, score, axis=1, device=device
            )
            return metrics
    ...
    def create_embedding(self, explainer, output_step="embedding"):
        ...
        with _device_mode_context(explainer.item_tower) as model:
            if "cls" in ds:
                ...
                ds = ds.map(
                    model.to_map_fn("inputs", output_step), batched=True, batch_size=64
                )
            else:
                ds = ds.map(
                    model.to_map_fn("text", output_step), batched=True, batch_size=64
                )
    ...
    def retrieve_similar(self, item_id, explainer, prior_score=None, topk=4):
        ...
        item_scores = (
            auto_cast_lazy_score(query_emb) @ item_emb.T
    ...
    def run_shap(self, item_id=None, cand_items=None, model_main="run_bmt_main"):
        *_, bmt = getattr(self, model_main)()
        ...
        if cand_items is None:
            cand_items = self.retrieve_similar(
                item_id, explainer
            ).ravel()  # a list of topk items
