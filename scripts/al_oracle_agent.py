...
from train_bmt_msmarco import (
    load_corpus,
    load_query,
    load_item_df, load_user_df,
    load_expl_response
)

from ms_marco_eval import (
    ranking,
    load_data
)
...
# %%
# train model
def training(train_dataset, epochs, model_checkpoint, save_dir, corpus=None, queries=None):
...
def generate_train_data(qids, qrels, ranking_profile, ranking_profile_2=None, num_of_samples_from_model=4):
    train_data = dict()
    for qid in qids:
...
def generate_train_data_with_accu_level(train_data, accu_level, qrels):
    ...
    print("number of correct samples: {} / {}".format(num_of_correct_data, len(train_data)))
...
def main(opt):
    ...
    qids_split = [qids_all[x:x+batch_size] for x in range(0, num_of_train_data, batch_size)]
    ...
    bm25_dir = "hotpotqa_results_oracle_agent_{}/data_iteration_0/ranking_profile_bm25.pt".format(accu_level)
    ...
    model_init_dir = "hotpotqa_results_oracle_agent_{}/data_iteration_0/model/checkpoints/epoch=4-step=20800.ckpt".format(accu_level)
    ...
    for ite in range(n_steps):
        for step in range(number_of_qid_split_batch):
            ...
            previous_working_dir = "hotpotqa_results_oracle_agent_{}/data_iteration_{}".format(accu_level, step_-1)
            current_working_dir = "hotpotqa_results_oracle_agent_{}/data_iteration_{}".format(accu_level, step_)
            ...
            train_data_oracle = generate_train_data(qids_split[step], qrels, ranking_profile, ranking_profile_bm25, num_of_samples_from_model)
            
            train_data = generate_train_data_with_accu_level(train_data_oracle, accu_level, qrels)
            
            if step_ > 0:
                train_data_prev_dir = os.path.join(previous_working_dir, "training_data.pt")
            ...
            bertmt = training(train_data, epochs, model_checkpoint=model_init_dir, save_dir=save_train_dir, corpus=corpus, queries=queries)
            model = bertmt.model
