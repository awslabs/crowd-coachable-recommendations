...
def create_simple_dataset(sign=1):
    ...
    V = create_dataset_unbiased(event_df, pd.concat([user_df for _ in range(5)]),
                                item_df, exclude_train=True)
...
def test_oracle(model_factory):
    ...
    testing_env = env.HoldoutEnv(user_df, item_df, event_df, clear_future_events=True,
                                 oracle=D, sample_size=1, recording=False)
    ...
    assert reward_by_policy['0'] > reward_by_policy['1'], f"insufficient test power {reward_by_policy}"
    assert reward_by_policy['0'] > 0.7, f"insufficient accuracy {reward_by_policy}"


@pytest.mark.flaky(max_runs=2, min_passes=1)
@pytest.mark.skipif(not torch.cuda.is_available(), reason="cuda not available")
@pytest.mark.parametrize("working_model_cls", [
    empirical_average_factory,  # graph_conv_factory
])
def test_simplest(working_model_cls, n_steps=20, test_every=1, epsilon=0.5,
    ...
    event_df = pd.DataFrame([(i, i, 0, 1) for i in item_df.index],
                            columns=['USER_ID', 'ITEM_ID', 'TIMESTAMP', 'VALUE'])
    user_df = event_df.set_index('USER_ID').assign(TEST_START_TIME=1)[['TEST_START_TIME']]
    ...
    iexp = InteractiveExperiment(
        ...
        [working_model], [oracle_model, baseline_model],
        epsilon=epsilon)
    iexp.run(n_steps, test_every)

    reward_by_policy = pd.DataFrame(iexp.testing_env._reward_by_policy).iloc[n_steps // 2:].mean(axis=0)
    print(reward_by_policy)
    pl.imshow(
        iexp.training_env.event_df.query('TIMESTAMP > 1').groupby(
            ['USER_ID', 'ITEM_ID'])['VALUE'].mean().unstack())
    pl.colorbar()
