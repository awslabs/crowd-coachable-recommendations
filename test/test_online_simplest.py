...
def test_run(
    ...
    event_df = pd.Series(item_df.index.values).to_frame('ITEM_ID').assign(
        USER_ID=np.arange(len(item_df)), TIMESTAMP=0)
    ...
    iexp = InteractiveExperiment(
        D.user_df, D.item_df, D.event_df,
    ...
    print(pd.DataFrame(iexp.testing_env._reward_by_policy).mean(axis=0))  # expect values=[0, 1, 0]
    print(iexp.training_env.event_df.groupby(['USER_ID', 'ITEM_ID'])['VALUE'].size().unstack())
    print(iexp.training_env.event_df.groupby(['USER_ID', 'ITEM_ID'])['VALUE'].mean().round(1).unstack())
