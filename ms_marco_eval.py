...
def generate_embeddings(data_indices, data_dic, embedding_func, batch_size, embedding_size=768, name=None):
    ...
    with torch.no_grad():
        for step in range(num_batches):
            ...
            embeddings[step*batch_size:(step*batch_size+len(indices)), :] = embedding_batch.cpu()
    if name is not None:
...
def ranking(data_dev, corpus, queries, embedding_func, batch_size):
    ...
    queries_embeddings = generate_embeddings(queries_ids, queries, embedding_func, batch_size, embedding_size=embedding_dim)
    passage_embeddings = generate_embeddings(corpus_ids, corpus, embedding_func, batch_size, embedding_size=embedding_dim)
    ...
    for step_q in range(num_query_batches):
        ...
        queries_embeddings_batch = queries_embeddings[step_q*batch_size:(step_q+1)*batch_size]
        ...
        for step_p in range(num_passage_batches):
            passage_embeddings_batch = passage_embeddings[step_p*batch_size:(step_p+1)*batch_size]
            ...
            scores = torch.mm(queries_embeddings_batch, passage_embeddings_batch.transpose(0, 1))
            scores = scores.cpu()
            ranking_matrix[0:num_of_rows, step_p*batch_size:(step_p*batch_size+num_of_cols)] = scores
...
def re_ranking(data_dev, corpus, queries, embedding_func, batch_size):
    ...
    queries_embeddings = generate_embeddings(queries_ids, queries, embedding_func, batch_size, embedding_size=embedding_dim)
    passage_embeddings = generate_embeddings(corpus_ids, effective_corpus, embedding_func, batch_size, embedding_size=embedding_dim)
...
def extract_hard_negatives(embedding_func, corpus, queries, model_name, batch_size=256, queries_embeddings=None, passage_embeddings=None):
    embedding_dim = 768
    ...
    if (queries_embeddings == None) or (passage_embeddings == None):
        ...
        queries_embeddings = generate_embeddings(queries_ids, queries, embedding_func, batch_size, embedding_size=embedding_dim, name=name_q)
        passage_embeddings = generate_embeddings(corpus_ids, corpus, embedding_func, batch_size, embedding_size=embedding_dim, name=name_p)

    # construct training dataset from 400M data
    print('Construct training dataset from 400M data')
    all_training_data = pd.read_csv('data/ms_marco/qidpidtriples.train.full.2.tsv', sep='\t', header=None, names=['qid', 'pos_pid', 'neg_pid'])
    ...
    for step, qid in enumerate(all_training_pos.keys()):
        ...
        embedding_qid , embedding_neg_pids = embedding_qid.cuda(), embedding_neg_pids.cuda()
...
def main(args):
    ...
    tokenize_func = lambda x: tokenizer(x, padding='max_length',max_length=300, truncation=True, return_tensors='pt')
    ...
    if model_type == 'automodel':
        ...
        def transform(x):
            ...
            model_outputs = model(input_ids=tokens['input_ids'].cuda(), attention_mask=tokens['attention_mask'].cuda())
    ...
    with autocast():
        if eval_method == 'ranking':
            ranking_profile = ranking(dev_data, corpus, query_data, embedding_func, batch_size)
        elif eval_method == 'reranking':
            ranking_profile = re_ranking(dev_top1000_data, corpus, query_data, embedding_func, batch_size)
            dev_data = process_dev_data_for_reranking(dev_data, dev_top1000_data)
        elif eval_method == 'find_hard_negatives':
            extract_hard_negatives(embedding_func, corpus, query_data, model_type, batch_size)
    time_end = time.time()
